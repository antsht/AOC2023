...
const char* categoryNames[] = {"seed", "soil", "fertilizer", "water", "humidity", "light", "temperature", "location" };
...
int main()
{
    ...
    c_temperature_to_humidity = read_converter_from_file("5temperature-to-humidity.txt", temperature, humidity);
    c_humidity_to_location = read_converter_from_file("5humidity-to-location.txt", humidity, location);
    
    register unsigned long int min_location = convert_seed_to_location(seeds[0]);
    for (int i = 0; i < 20; i = i + 2)
    {
        for ( unsigned long int j = 0; j < seeds[i+1]; ++j )
        {
        register unsigned long int current_location = convert_seed_to_location(seeds[i]+j);
        if (current_location < min_location)
            min_location = current_location;
        }
        printf("I: %d  Corrent_min: %lu \n", i, min_location);
    }
    ...
    if (fgets(buffer, 256, fp))
        {
            ...
            printf("%s to %s N: %d Dst: %lu Src: %lu Range: %lu\n", categoryNames[converter.src_t], categoryNames[converter.dst_t], converter.converters_cnt, converter.converter_rows[i].dst, converter.converter_rows[i].src, converter.converter_rows[i].range );
            i++;            
            //getchar();
        }
    }
...
unsigned long int convert_value(unsigned long int src_val, converter_t converter)
{
    ...
    {
        if (src_val >= converter.converter_rows[i].src && src_val < converter.converter_rows[i].src + converter.converter_rows[i].range)
            {
