...
int main() {
    ...
    register unsigned long int min_location = convert_seed_to_location(
    ...
    for (int i = 0; i < 20; i = i + 2) {
        for (unsigned long int j = 0; j < seeds[i + 1]; ++j) {
            register unsigned long int current_location = convert_seed_to_location(
                seeds[i] + j, c_seed_to_soil, c_soil_to_fertilizer, c_fertilizer_to_water, c_water_to_light,
                c_light_to_temperature, c_temperature_to_humidity, c_humidity_to_location);
