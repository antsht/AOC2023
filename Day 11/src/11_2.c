...
void count_empty_rows_cols() {
    ...
    for (int r = 0; r < ROWS_COUNT; ++r) {
        if (stars_in_row[r] == 0) empty_rows_cnt++;
    }
    for (int c = 0; c < COLS_COUNT; ++c) {
        if (stars_in_col[c] == 0) empty_cols_cnt++;
    }
}
...
void fill_expanded_galaxy() {
    ...
    while (s_row < ROWS_COUNT) {
        while (s_col < COLS_COUNT) {
            ...
            if (stars_in_col[s_col] == 0) t_col++;
            t_col++;
            s_col++;
        }
        s_col = 0;
        t_col = 0;
        if (stars_in_row[s_row] == 0) t_row++;
        t_row++;
        s_row++;
    }
}
...
long long calc_distance(star_t star1, star_t star2) {
    ...
    if (star1.row == star2.row && star1.col == star2.col) return 0;
    if (star1.row <= star2.row) {
        for (int t = star1.row; t < star2.row; t++) distance += expanded_galaxy[t][star1.col].distance;
    } else {
        for (int t = star2.row; t < star1.row; t++) distance += expanded_galaxy[t][star1.col].distance;
    }
    if (star1.col <= star2.col) {
        for (int t = star1.col; t < star2.col; t++) distance += expanded_galaxy[star1.row][t].distance;
    } else {
        for (int t = star2.col; t < star1.col; t++) distance += expanded_galaxy[star1.row][t].distance;
    }
