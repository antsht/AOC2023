    ...
    while (!feof(fp)) {
        if (fgets(buffer, 25000, fp)) {
            ...
            while (istr != NULL) {
                card_t card;
                int namelen = 0;
                while (istr[namelen] != '-' && istr[namelen] != '=' && istr[namelen] != '\0' &&
                       istr[namelen] != '\n') {
...
void remove_card(card_t card) {
    ...
    for (int i = 0; i < hashmap[h].len; ++i) {
        if (strcmp(hashmap[h].cards[i].name, card.name) == 0) strcpy(hashmap[h].cards[i].name, "\0");
    }
}
        ...
        if (hashmap[i].len > 0) {
            ...
            for (int n = 0; n < hashmap[i].len; ++n) {
                if (hashmap[i].cards[n].name[0] != '\0')
                    printf("Card %d: name=%s | val=%d\n", n, hashmap[i].cards[n].name,
                           hashmap[i].cards[n].value);
            }
        }
    }
}
