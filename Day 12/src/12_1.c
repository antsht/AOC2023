...
void generate_strings_optimized(char *current, int target_length, int current_length, int row_n) {
    ...
    for (int i = 0; i < dot_count; i++) {
        ...
        if (dot_positions[i] != current_length) {
            memmove(current + dot_positions[i] + 2, current + dot_positions[i] + 1,
                    current_length - dot_positions[i]);
        ...
        // Remove the inserted '.'
        if (dot_positions[i] != current_length) {
            memmove(current + dot_positions[i] + 1, current + dot_positions[i] + 2,
                    current_length - dot_positions[i]);
...
int strpatcmp(char *pattern, char *strtocompare) {
    ...
    while (pattern[i] != '\0' && strtocompare[i] != '\0') {
        if (strtocompare[i] != pattern[i] && pattern[i] != '?') return 0;
        i++;
    }
    return res;
}
...
void clear_tmp() {
    for (int i = 0; i < 100000; ++i) tmprecords[i][0] = '\0';
    return;
}

int check_perm(char *strtocheck) {
    for (int i = 0; i < cur_matches_cnt; i++)
        if (strcmp(tmprecords[i], strtocheck) == 0) return 1;
    return 0;
}